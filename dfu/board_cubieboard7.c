 ...
 * adfu.c
 *
 * Device Firmware Upgrating for cubieboard7.
 * Refer to https://github.com/96boards-cubieboard7/linaro-adfu-tool/
 *
...
static int cb7_uninit_usb(void)
{
	DEBUG("%s\n", __func__);
 ...
 * You should call cb7_init_usb() first before using this function
 *
 * @return the handler of the device
 */
static libusb_device_handle *cb7_init_device(void)
{
	int i;
...
static void cb7_uninit_device(libusb_device_handle *handler)
{
	int r1;
...
static int cubieboard7_init(struct dfu_board *board)
{
	DEBUG("%s\n", __func__);

	cb7_init_usb();

	libusb_handler = cb7_init_device();
	if (libusb_handler == NULL) {
		printf("cb7_init_device failed\n");
		return -1;
	}
...
static int cubieboard7_exit(struct dfu_board *board)
{
	DEBUG("%s\n", __func__);

	if (libusb_handler != NULL) {
		cb7_uninit_device(libusb_handler);
		libusb_handler = NULL;
	}
...
static int cubieboard7_upload(struct dfu_board *board, const char *filename,
			    unsigned int addr)
{
...
static int cubieboard7_run(struct dfu_board *board, unsigned int addr)
{
	DEBUG("%s, addr 0x%x\n", __func__, addr);
...
static struct dfu_board board_cubieboard7 = {
	.board_name = "cubieboard7",

	.init = cubieboard7_init,
	.exit = cubieboard7_exit,
	.upload = cubieboard7_upload,
	.run = cubieboard7_run,
};

__attribute__((constructor)) void board_cubieboard7_init(void)
{
	DEBUG("%s\n", __func__);

	dfu_board_register(&board_cubieboard7);
}
